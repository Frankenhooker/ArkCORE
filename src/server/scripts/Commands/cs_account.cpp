...
public:
    ...
    static bool HandleAccountOnlineListCommand (ChatHandler* handler, const char* /*args*/)
    {
        ...
        do
        {
            ...
            QueryResult resultLogin = LoginDatabase.PQuery("SELECT a.username, a.last_ip, aa.gmlevel, a.expansion "
                    "FROM account a "
            ...
            if (resultLogin)
            {
                ...
                handler->PSendSysMessage(LANG_ACCOUNT_LIST_LINE, fieldsLogin[0].GetCString(), name.c_str(), fieldsLogin[1].GetCString(), fieldsDB[2].GetUInt16(), fieldsDB[3].GetUInt16(), fieldsLogin[3].GetUInt32(), fieldsLogin[2].GetUInt32());
            }
            else
    ...
    static bool HandleAccountSetAddonCommand (ChatHandler* handler, const char *args)
    {
        ...
        // or to self account
        if (handler->GetSession() && handler->GetSession()->GetAccountId() != account_id && handler->HasLowerSecurityAccount(NULL, account_id, true))
            return false;
